...
store_path = '../TwoTimeScale_data/exmp51/' 
...
for alpha in alpha_list:
    for beta in beta_list:
        ...
        print(eta_ini1, eta_ini2, grid_infor[eta_ind, 3])
        x_list = []
        y_list = []
        x_inner_list = []
        bias_max_list = []
        ...
        for i in range(n_ite):
            ...
            # bias_max_list.append(np.max(bias ** 2) / eta_beta)
            # bias_ave_list.append(np.linalg.norm(bias) ** 2 / eta_beta)
        np.save(path + 'x.npy', np.array(x_list))
        np.save(path + 'y.npy', np.array(y_list))
        np.save(path + 'x_inner.npy', np.array(x_inner_list))
        # np.save(path + 'bias_max.npy', np.array(bias_max_list))
        # np.save(path + 'bias_ave.npy', np.array(bias_ave_list))